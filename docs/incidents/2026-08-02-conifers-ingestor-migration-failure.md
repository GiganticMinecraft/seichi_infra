...
| 4 | migration のリリース単位 Job 化 | 影響拡大要因 | **採用**(DB ユーザー分離は 2026-08-03 に不採用へスコープ変更、表下の記録参照) | conifers + infra 担当 | 設計 2026-08-10・実装 2026-08-23 | [infra#5693](https://github.com/GiganticMinecraft/seichi_infra/issues/5693) | 5 分ごとの init 実行を廃止しリリース時 1 回の専用 Job に分離。要設計: migration 完了→新 ingestor 配備の順序保証、旧版互換、migration 失敗時に旧 ingest が継続すること |
...
スコープ変更の記録(2026-08-03): 対策 4 に当初含めていた **DB ユーザー分離(migration=DDL / ingestor=DML 限定)は不採用**とした。理由: この DB は conifers 専用で書き手は ingestor のみであり、DML 権限だけでも全データ破壊(DELETE/UPDATE)は可能なため、DDL を剥がして守れる差分はスキーマ破壊のみで、どちらのケースも復旧手段はバックアップからのリストアで同一。対策 3(純 Rust migrator 分離)の実施により ingestor のコードパスに DDL は存在しない。一方でユーザー分離には Terraform でのユーザー・Secret 追加、grant 管理、ローテーション対象の増加という継続コストがある。共有 DB に同居する構成へ変わる場合は再検討する。
