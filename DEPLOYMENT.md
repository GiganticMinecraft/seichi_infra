...
整地鯖の本番環境は9台のベアメタルマシンで稼働しており、それぞれの上に [Proxmox](https://www.proxmox.com/en/) が搭載されています。

ハードウェアの構成については詳しくは説明しませんが、合計するとおよそ **208 core vCPUs**、**約960GB memory** のリソースがあります。ネットワークストレージは TrueNAS 3 台と Synology NAS 2 台が iSCSI / NFS で合計 20TiB 超のボリュームを提供しており、VM のディスクおよび Kubernetes の永続ストレージ(CSI 経由)として利用できます。バックアップは専用の Proxmox Backup Server(PBS)に集約されています。
...
旧来の VM 方式です。かつては独自プラグインを複数含むマイクラ鯖の運用方針がこの方法でしか確立されていませんでしたが、現在は本番のマイクラ鯖群も Kubernetes 上(`seichi-minecraft` namespace)で稼働しています。
VM として残っているのは Proxmox Backup Server、NetBox、Redmine、Nextcloud、旧監視基盤(prometheus-production)などの一部インフラ用途のみです。基本的には必要がない限り新しい VM を作ることはないため、このガイドでも大きくは取り上げないこととします。
