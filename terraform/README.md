...
2026/07/08現在、Terraform 管理下にある設定および定義は以下の通りです。ドキュメントは古くなっている可能性があるため、詳細はこのディレクトリ内にある各 `.tf` ファイルを参照してください。

 - GitHub
   - Teamsの定義
   - Repositoryの設定
 - Cloudflare
   - Zero Trust Access Applicationのアクセス制御設定（GitHub OAuthをIdPとする管理系エンドポイント群）
   - Edge証明書の設定
   - Page ruleの設定
   - DNS設定
   - Pagesプロジェクト（seichi-portal）
 - オンプレ環境の k8s クラスタ上の一部リソースの管理
   - `Secret` リソースと `Namespace` リソース
   - ArgoCD 本体のブートストラップ（`helm_release`。ArgoCD 以外のアプリケーションは ArgoCD が Pull 型で同期するため Terraform 管理外）
...
このセットアップを実現するためには、 `terraform apply` 等を実行するマシン上で `cloudflared access` によるトンネルが作成される必要があります。Terraform Cloud 上ではそのような設定 (`apply` / `plan` の前後で追加のコマンドを走らせるといった指定) は 2022/06/04 時点では不可能だったため、 GitHub Actions ワークフロー上で実現し、Terraform Cloudには `.tfstate` の管理だけを任せています（この制約が現在の HCP Terraform にも当てはまるかは再検証していませんが、現構成で問題なく運用できています）。
