...
resource "cloudflare_access_application" "onp_seichi_timed_stats_influxdb_ui" {
  zone_id          = local.cloudflare_zone_id
  name             = "seichi-timed-stats InfluxDB UI"
  domain           = "seichi-timed-stats-influxdb-ui.onp-k8s.admin.${local.root_domain}"
  type             = "self_hosted"
  session_duration = "24h"
...
resource "cloudflare_access_policy" "onp_seichi_timed_stats_influxdb_ui" {
  application_id = cloudflare_access_application.onp_seichi_timed_stats_influxdb_ui.id
  zone_id        = local.cloudflare_zone_id
  name           = "Require to be in a GitHub team to access"
  precedence     = "1"
  decision       = "allow"
  ...
  include {
    github {
      name                 = local.github_org_name
      teams                = [github_team.onp_seichi_timed_stats_influxdb_ui.slug]
      identity_provider_id = cloudflare_access_identity_provider.github_oauth.id
    }
  }
}
